// WARNING: DO NOT EDIT THIS FILE. THIS FILE IS MANAGED BY SPRING ROO.
// You may push code into the target .java compilation unit if you wish to edit any member(s).

package com.raze.coleadmin.controller;

import com.raze.coleadmin.catalog.NivelEducativo;
import com.raze.coleadmin.controller.NivelEducativoController;
import com.raze.coleadmin.domain.Escuela;
import com.raze.coleadmin.service.EscuelaService;
import com.raze.coleadmin.service.NivelEducativoService;
import com.raze.coleadmin.service.TipoNivelEducativoService;
import com.raze.coleadmin.service.UsuarioService;
import java.io.UnsupportedEncodingException;
import java.util.ArrayList;
import java.util.List;
import javax.servlet.http.HttpServletRequest;
import javax.validation.Valid;
import org.joda.time.format.DateTimeFormat;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.context.i18n.LocaleContextHolder;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.util.UriUtils;
import org.springframework.web.util.WebUtils;

privileged aspect NivelEducativoController_Roo_Controller {
    
    @Autowired
    EscuelaService NivelEducativoController.escuelaService;
    
    @Autowired
    NivelEducativoService NivelEducativoController.nivelEducativoService;
    
    @Autowired
    TipoNivelEducativoService NivelEducativoController.tipoNivelEducativoService;
    
    @Autowired
    UsuarioService NivelEducativoController.usuarioService;
    
    @RequestMapping(method = RequestMethod.POST, produces = "text/html")
    public String NivelEducativoController.create(@Valid NivelEducativo nivelEducativo, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, nivelEducativo);
            return "niveleducativoes/create";
        }
        uiModel.asMap().clear();
        nivelEducativoService.saveNivelEducativo(nivelEducativo);
        return "redirect:/niveleducativoes/" + encodeUrlPathSegment(nivelEducativo.getId().toString(), httpServletRequest);
    }
    
    @RequestMapping(params = "form", produces = "text/html")
    public String NivelEducativoController.createForm(Model uiModel) {
        populateEditForm(uiModel, new NivelEducativo());
        List<String[]> dependencies = new ArrayList<String[]>();
        if (escuelaService.countAllEscuelas() == 0) {
            dependencies.add(new String[] { "tipoNivelEducativo", "escuelas" });
        }
        uiModel.addAttribute("dependencies", dependencies);
        return "niveleducativoes/create";
    }
    
    @RequestMapping(value = "/{id}", produces = "text/html")
    public String NivelEducativoController.show(@PathVariable("id") Long id, Model uiModel) {
        addDateTimeFormatPatterns(uiModel);
        uiModel.addAttribute("niveleducativo", nivelEducativoService.findNivelEducativo(id));
        uiModel.addAttribute("itemId", id);
        return "niveleducativoes/show";
    }
    
    @RequestMapping(produces = "text/html")
    public String NivelEducativoController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("niveleducativoes", nivelEducativoService.findNivelEducativoEntries(firstResult, sizeNo));
            float nrOfPages = (float) nivelEducativoService.countAllNivelEducativoes() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("niveleducativoes", nivelEducativoService.findAllNivelEducativoes());
        }
        addDateTimeFormatPatterns(uiModel);
        return "niveleducativoes/list";
    }
    
    @RequestMapping(method = RequestMethod.PUT, produces = "text/html")
    public String NivelEducativoController.update(@Valid NivelEducativo nivelEducativo, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, nivelEducativo);
            return "niveleducativoes/update";
        }
        uiModel.asMap().clear();
        nivelEducativoService.updateNivelEducativo(nivelEducativo);
        return "redirect:/niveleducativoes/" + encodeUrlPathSegment(nivelEducativo.getId().toString(), httpServletRequest);
    }
    
    @RequestMapping(value = "/{id}", params = "form", produces = "text/html")
    public String NivelEducativoController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, nivelEducativoService.findNivelEducativo(id));
        return "niveleducativoes/update";
    }
    
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = "text/html")
    public String NivelEducativoController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        NivelEducativo nivelEducativo = nivelEducativoService.findNivelEducativo(id);
        nivelEducativoService.deleteNivelEducativo(nivelEducativo);
        uiModel.asMap().clear();
        uiModel.addAttribute("page", (page == null) ? "1" : page.toString());
        uiModel.addAttribute("size", (size == null) ? "10" : size.toString());
        return "redirect:/niveleducativoes";
    }
    
    void NivelEducativoController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("nivelEducativo_fechacreacion_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
        uiModel.addAttribute("nivelEducativo_fechamodificacion_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
    
    void NivelEducativoController.populateEditForm(Model uiModel, NivelEducativo nivelEducativo) {
        uiModel.addAttribute("nivelEducativo", nivelEducativo);
        addDateTimeFormatPatterns(uiModel);
        uiModel.addAttribute("tiponiveleducativoes", tipoNivelEducativoService.findAllTipoNivelEducativoes());
        uiModel.addAttribute("escuelas", escuelaService.findAllEscuelas());
        uiModel.addAttribute("usuarios", usuarioService.findAllUsuarios());
    }
    
    String NivelEducativoController.encodeUrlPathSegment(String pathSegment, HttpServletRequest httpServletRequest) {
        String enc = httpServletRequest.getCharacterEncoding();
        if (enc == null) {
            enc = WebUtils.DEFAULT_CHARACTER_ENCODING;
        }
        try {
            pathSegment = UriUtils.encodePathSegment(pathSegment, enc);
        } catch (UnsupportedEncodingException uee) {}
        return pathSegment;
    }
    
}
